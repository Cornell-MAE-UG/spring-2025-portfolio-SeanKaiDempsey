...
technologies: [Solidworks, ANSYS FEA, Haas 3-axis CNC]
...
![TG21 Driveshafts]({{ "/assets/images/driveshafts.png" | relative_url }}){: style="width: 600px; display: block; margin: 0 auto;"}
...
- Must fit within tight packaging constraints set by the CV joints, links, and frame
...
The final design uses **solid billet Ti 6Al-4V STA for the front** shafts (lower load case, significant weight savings) and **gun-drilled 300M steel for the rear** (higher load case requires the stronger material). The front shafts save 1.38 lbs each (48.6% reduction) and the rear shafts save up to 1.58 lbs each (53.4% reduction), for a total combined saving of approximately **5.92 lbs**.

## Ansys FEA

![Driveshaft Drop Test Load Case]({{ "/assets/images/PLACEHOLDER_driveshaft_ansys_drop.png" | relative_url }}){: style="width: 600px; display: block; margin: 0 auto;"}

The primary load case analyzed in Ansys was the drop test — the car being dropped onto a high traction surface with the driveline spun up, producing a sudden shock torque. I set up the Ansys model to apply this torsional load and verify that stresses stayed within material limits. Stress concentrations from the fillets at diameter transitions were also investigated; the fillet radii were large enough relative to the shaft diameter that standard table-based stress concentration factors weren't applicable, making the Ansys validation particularly important.
...
![Plunging Cup Shock Torque Ansys]({{ "/assets/images/PLACEHOLDER_plunging_ansys.png" | relative_url }}){: style="width: 600px; display: block; margin: 0 auto;"}
...
The front plunging cups integrate with the front differential and had the most packaging challenges. A shimming oversight early in the design caused clearance issues between the cup and the diff/gearbox — fixing this required moving the abutment face and carefully re-verifying that enough material remained for axial load transfer. On the other side, the diff bolt circle was too close to the cup OD, requiring the outer diameter to be tapered down to avoid interference. I also standardized the bolt pattern to match TG19's retaining plate holes, allowing the front and rear cups to share the same retaining plates.

### Rear Plunging Cups

![Rear Plunging Cup]({{ "/assets/images/PLACEHOLDER_rear_plunging.png" | relative_url }}){: style="width: 500px; display: block; margin: 0 auto;"}

For the rear plunging cups I added a key slot to the brake rotor mount interface. When tightening the rotor mount onto the cup, the mount seats into the groove, preventing any axial movement of the rotor mount during competition loading — a simple but important fix for rotor retention reliability.

## Competition Results

TG21 competed successfully with no issues on either the driveshafts or plunging cups. A plunging CV internal cage failed during competition, but this was an off-the-shelf component, not a designed part. The diff plunging cups ran all competition without issue.
