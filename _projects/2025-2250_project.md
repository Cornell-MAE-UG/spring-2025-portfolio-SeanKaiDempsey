...
Once we had identified the issue we were trying to tackle we worked on brainstorming ideas, features and designs to accomplish the goal. After collecting many ideas—some more practical than others— we moved on to the initial CAD phase.

![Can Cap 3D printed model with O-ring]({{ "/assets/images/2250-CanCap-Drawing.png" | relative_url }}){: .inline-image-r style="width: 360px"}

We landed on a design that would fit snugly around a can with internal tapers so that it would fit universally with many can designs. It would secure onto the can by an external nut locking the can cap into place.
